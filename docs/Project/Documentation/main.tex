...
Bayesian risk integration; and (iv) validation, calibration, and
compliance assessment. On the OPP-115 corpus, a fine-tuned
...
\begin{itemize}
  ...
  and an expected calibration error of $0.0525$, with TF-IDF
  logistic-regression and na\"ive-Bayes ablations
  ...
  with acceptance checks, synthetic policy/schema fixtures, and a Gradio web
  interface for local or public review
...
into standards-traceable risk posteriors.
...
third-party sharing, and personally identifiable information protection, and
a Gradio web demonstrator exposes the full pipeline interactively for local
or temporary public review~\cite{gradio_docs,huggingface_spaces_docs}.
...
Table~\ref{tab:modelcomp} reports the three classifiers on the held-out
validation and test partitions. The July~2026 PrivBERT freeze achieves a test
macro-F1 of $0.869$ and accuracy of $0.954$, improving on the same-split
na\"ive-Bayes baseline by $21.9$ macro-F1 points and $13.3$ accuracy points.
The archived TF-IDF logistic-regression ablation is reported for technique
coverage but was produced on an earlier policy split, so it is not used as a
same-split acceptance comparison. The gap is most pronounced on the minority
classes, where contextual representations are decisive.

\begin{table}[t]
\centering
\caption{Classifier comparison on OPP-115 (macro-F1 and accuracy).}
\label{tab:modelcomp}
\footnotesize
...
\midrule
\textbf{PrivBERT freeze}          & \textbf{0.935} & \textbf{0.973} & \textbf{0.869} & \textbf{0.954} \\
Na\"ive Bayes (same split)        & 0.704 & 0.842 & 0.649 & 0.821 \\
LogReg TF-IDF (archived split)    & 0.758 & 0.890 & 0.779 & 0.893 \\
\bottomrule
\end{tabular}
\end{table}
...
rather than artefacts of a single favourable split.
...
standards-traceable probabilistic scoring can be productively unified. Three
...
mitigates but does not eliminate this effect. Third, traceability is a
...
licence. The framework is intended as a decision-support and transparency aid;
its scores are advisory and should not substitute for qualified legal
...
trustworthiness. Future work will pursue three directions: domain-adaptive

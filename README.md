...
|                |
|  **Total Sums**  | **70 hrs 11 mins** | **95 hrs** |
|                |
|     **Total**      | **165 hrs 11 mins** |                     |
...
```bash
PYTHONPATH=src python scripts/run_phase4_web.py --port 7860
```

Equivalent package command:

```bash
prert-phase4-web --port 7860
```
...
For an internet-accessible professor demo, request a temporary Gradio share URL:

```bash
prert-phase4-web --public
```
...
## 13) Gradio Public Demo URL

Run the local Gradio app with a temporary public share URL:

```bash
prert-phase4-web --public
```

For a persistent hosted demo, publish the Gradio Space bundle generated by `scripts/prepare_huggingface_release.py` and share the Hugging Face Space URL.

## 13.1) Prepare Hugging Face Release Bundle

> [!NOTE]
> Detailed instructions in [huggingface/README.md](huggingface/README.md)

First run the prepare huggingface release script to generate bundle:

```bash
python scripts/prepare_huggingface_release.py   --checkpoint artifacts/phase-3-privacybert/classifier_checkpoint/privacybert   --model-output dist/huggingface/model   --space-output dist/huggingface/space   --model-id <account name>/PrERT-CNM-v4-privacybert   --space-title "PrERT-CNM Compliance Studio"
```

After that, use the `hf login` command to authenticate your account. Once logged in, run the following commands to push the model and Space to Hugging Face:

```bash
# Space
hf upload <account name>/PrERT-CNM-v4-privacybert-demo dist/huggingface/space --repo-type space

# Model
hf upload <account name>/PrERT-CNM-v4-privacybert dist/huggingface/model --repo-type model
```
